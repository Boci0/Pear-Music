...
void main() {
  test('colour transition fades via ColorScheme.lerp (no snap / red screen)',
      () {
    final a = buildTheme(ArtworkPalette.fallback);
    final b = buildTheme(const Color(0xFF4CAF50));

    // Exactly what the transition does each frame: lerp the scheme, rebuild.
    final mid = PlayerTheme.buildFromScheme(
        ColorScheme.lerp(a.colorScheme, b.colorScheme, 0.5));
    final end = PlayerTheme.buildFromScheme(
        ColorScheme.lerp(a.colorScheme, b.colorScheme, 1.0));
